...
const Day = (props: DayProps) =>{
    return(
        <VStack flexCross={false} flexMain={false} width={20} height={30} style={props.boxStyle}>
        ...
        <HStack flexMain={false} flexCross={false}  >
            {
                props.daysInRow.map( (num, idx) =>{
                    return <Day key={idx} day={`${num}`} textStyle={props.textStyle} boxStyle={{backgroundColor: 'red'}} />
                })
            }
        </HStack>
    )
...
export const Calander = (props : CalanderProps) => {
  ...
  return (
    <VStack style={{backgroundColor: "#000000"}} align='flex-start'>
        <CalanderRow daysInRow={[1,2,3,4,5,6,7]} textStyle={props.textStyle!} />
        <CalanderRow daysInRow={[1,2,3,4,5,6,7]} textStyle={props.textStyle!} />
        <CalanderRow daysInRow={[1,2,3,4,5,6,7]} textStyle={props.textStyle!} />
        <CalanderRow daysInRow={[1,2,3,4,5,6,7]} textStyle={props.textStyle!} />
...
export const StyledCalander = withStyle<CalanderProps>(
    Calander,
    makeStyle
);
